...
class Player_Model_Based(Player):
    ...
    def generate_values_for_possible_actions(self,
        ...
        initial_vector = BoardVector(board=self.board, include_values = False)
        x1,x2,x3 = [],[],[]
        for action in possible_actions:
            if action[0] == None:
                new_vector = initial_vector.vector.copy()
            elif action[0] == 'street':
                new_vector = initial_vector.build_street(self.player_position,action[1]  )
            elif action[0] == 'village':
                new_vector = initial_vector.build_village(self.player_position,action[1]  )
            elif action[0] == 'town':
                new_vector = initial_vector.build_town(self.player_position,action[1]  )
            elif action[0] == 'trade_player':
                new_vector = initial_vector.trade_between_players(self.player_position,action[1]  )
            elif action[0] == 'trade_specific_player':
                new_vector = initial_vector.trade_between_players(self.player_position,action[1][0], action[1][1])
            elif action[0] == 'trade_bank':
                new_vector = initial_vector.trade_with_bank(self.player_position,action[1]  )
            else:
    ...
    def find_best_action(self, 
        ...
        if possible_actions:
            values = self.generate_values_for_possible_actions(possible_actions)[..., self.player_position]
    ...
    def player_setup(self,brd) -> None:
        ...
        values = self.generate_values_for_possible_actions(options)[..., self.player_position]
        best_index = np.argmax(values)
    ...
    def calculate_value(self, action: tuple = (None,None)):
        ...
        return values[0][self.player_position]
